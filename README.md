...
Click links in formatted view to open safe targets with the Windows default handler. Relative document links are resolved from the folder containing the opened Markdown file, internal `#heading` links scroll inside the reader, and local executable/script-like targets are blocked unless their extensions are added in Settings. Extension overrides are saved with the rest of the app settings.
